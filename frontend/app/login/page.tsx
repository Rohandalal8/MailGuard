...
export default function LoginPage() {
	...
	return (
		<main className="login">
			<section className="panel" style={{ width: "min(440px, calc(100% - 36px))" }}>
				<div className="eyebrow">MAILGUARD</div>
				<h1>Secure your inbox.</h1>
				<p className="muted">Sign in with Google, then explicitly connect the Gmail mailbox you want analyzed.</p>
				<button className="button" disabled={loading} onClick={() => void signInWithGoogle().then(() => router.push("/dashboard")).catch((reason: unknown) => setError(getAuthErrorMessage(reason)))}>
					Continue with Google
				</button>
				<p className="muted" style={{ fontSize: 13, lineHeight: 1.6, marginTop: 16, textAlign: "left" }}>
					By continuing, you agree to our <a href="/terms-of-service" style={{ color: "#16211d", fontWeight: 700 }}>Terms of Service</a> and <a href="/privacy-policy" style={{ color: "#16211d", fontWeight: 700 }}>Privacy Policy</a>.
				</p>
				{error && <p style={{ color: "#9c2d20", lineHeight: 1.5 }}>{error}</p>}
			</section>
		</main>
	);
}
