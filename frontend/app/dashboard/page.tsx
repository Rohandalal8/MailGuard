...
export default function DashboardPage() {
    ...
    async function refresh() {
        ...
        try {
            ...
            setStats(result);
            setRecent(result.recentEmails);
            setError("");
            window.dispatchEvent(new Event("dashboard-stats-updated"));
        } catch {
            setError("Unable to load dashboard data. Please refresh after signing in.");
        } finally {
    ...
    const values = statsLoading ? [["Total emails", "..."], ["Inbox", "..."], ["Spam", "..."], ["Scam", "..."]] : [["Total emails", stats?.total ?? 0], ["Inbox", stats?.INBOX ?? 0], ["Spam", stats?.SPAM ?? 0], ["Scam", stats?.SCAM ?? 0]];

    return (
        <AppShell>
            <div className="eyebrow">Overview</div>
            <h1>Your email perimeter.</h1>
            {error &&
